...
static void ehci_hsic_prevent_sleep(struct msm_hsic_hcd *mehci)
{
	if (!in_interrupt()) {
		pr_info("%s\n", __func__);
		pm_qos_update_request(&mehci->pm_qos_req_dma, msm_cpuidle_get_deep_idle_latency());
	}
}
...
static void ehci_hsic_allow_sleep(struct msm_hsic_hcd *mehci)
{
	if (!in_interrupt()) {
		pr_info("%s\n", __func__);
		pm_qos_update_request(&mehci->pm_qos_req_dma, PM_QOS_DEFAULT_VALUE);
	}
}
...
static int __devinit ehci_hsic_msm_probe(struct platform_device *pdev)
{
	...
	pm_qos_add_request(&mehci->pm_qos_req_dma, PM_QOS_CPU_DMA_LATENCY,
				PM_QOS_DEFAULT_VALUE);
	pr_info("%s[%d] pm_qos_add_request pm_qos_req_dma\n", __func__, __LINE__);
...
static int __devexit ehci_hsic_msm_remove(struct platform_device *pdev)
{
	...
	pr_info("%s[%d] pm_qos_remove_request pm_qos_req_dma\n", __func__, __LINE__);
