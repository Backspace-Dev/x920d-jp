...
static struct msm_actuator_info *ad5823_msm_actuator_info;

static int32_t ad5823_poweron_af(void)
{
	int32_t rc = 0;
	pr_info("%s enable AF actuator, gpio = %d\n", __func__,
			ad5823_msm_actuator_info->vcm_pwd);
	mdelay(1);
	rc = gpio_request(ad5823_msm_actuator_info->vcm_pwd, "ad5823");
	if (!rc)
		gpio_direction_output(ad5823_msm_actuator_info->vcm_pwd, 1);
	else
		pr_err("%s: AF PowerON gpio_request failed %d\n", __func__, rc);
	gpio_free(ad5823_msm_actuator_info->vcm_pwd);
	mdelay(1);
	return rc;
}

static void ad5823_poweroff_af(void)
{
	int32_t rc = 0;
	...
	rc = gpio_request(ad5823_msm_actuator_info->vcm_pwd, "ad5823");
	if (!rc)
		gpio_direction_output(ad5823_msm_actuator_info->vcm_pwd, 0);
	else
...
int32_t ad5823_msm_actuator_init_table(
	struct msm_actuator_ctrl_t *a_ctrl)
{
	...
	if (a_ctrl->step_position_table != NULL) {
		uint16_t i = 0;
		uint16_t ad5823_nl_region_boundary1 = 2;
		uint16_t ad5823_nl_region_code_per_step1 = 32;
		uint16_t ad5823_l_region_code_per_step = 16;
		uint16_t ad5823_max_value = 1023;
		...
		for (i = 1; i <= a_ctrl->set_info.total_steps; i++) {
			...
			if (i <= ad5823_nl_region_boundary1) {
				a_ctrl->step_position_table[i] =
					...
					+ ad5823_nl_region_code_per_step1;
			} else {
				a_ctrl->step_position_table[i] =
					a_ctrl->step_position_table[i-1]
					+ ad5823_l_region_code_per_step;
			}

			if (a_ctrl->step_position_table[i] > ad5823_max_value)
				a_ctrl->step_position_table[i] = ad5823_max_value;
			}
		}
...
int32_t ad5823_msm_actuator_move_focus(
	struct msm_actuator_ctrl_t *a_ctrl,
	int dir,
	int32_t num_steps)
{
...
int ad5823_actuator_af_power_down(void *params)
{
	int rc = 0;
	LINFO("%s called\n", __func__);

	rc = (int) msm_actuator_af_power_down(&ad5823_act_t);
	ad5823_poweroff_af();
	return rc;
}
...
int32_t ad5823_act_write_focus(
	struct msm_actuator_ctrl_t *a_ctrl,
...
static int32_t ad5823_act_init_focus(struct msm_actuator_ctrl_t *a_ctrl)
{
	int32_t rc = 0;
...
static const struct i2c_device_id ad5823_act_i2c_id[] = {
	{"ad5823_act", (kernel_ulong_t)&ad5823_act_t},
	{ }
};
	...
	void)
{
	...
	rc = ad5823_poweron_af();
	if (rc < 0) {
		pr_err("%s power on failed\n", __func__);
		return (int) rc;
	}

	ad5823_act_t.step_position_table = NULL;
	rc = ad5823_act_t.func_tbl.actuator_init_table(&ad5823_act_t);
	if (rc < 0) {
		pr_err("%s init table failed\n", __func__);
		return (int) rc;
	}

	rc = msm_camera_i2c_write(&(ad5823_act_t.i2c_client),
		0x0001, 0x01,
		MSM_CAMERA_I2C_BYTE_DATA);
	if (rc < 0) {
		pr_err("%s i2c write failed\n", __func__);
...
static int32_t ad5823_act_create_subdevice(
	void *board_info,
	void *sdev)
{
	LINFO("%s called\n", __func__);

	ad5823_msm_actuator_info = (struct msm_actuator_info *)board_info;

	return (int) msm_actuator_create_subdevice(&ad5823_act_t,
		ad5823_msm_actuator_info->board_info,
...
static struct msm_actuator_ctrl_t ad5823_act_t = {
	...
	.func_tbl = {
		...
		.actuator_move_focus = ad5823_msm_actuator_move_focus,
		.actuator_write_focus = ad5823_act_write_focus,
		.actuator_set_default_focus = msm_actuator_set_default_focus,
		.actuator_init_focus = ad5823_act_init_focus,
