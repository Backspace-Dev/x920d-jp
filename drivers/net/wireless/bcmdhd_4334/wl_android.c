...
static int wl_android_get_conap_channel(struct net_device *net, char *buf, int len)
{
    ...
    if ((error = wldev_ioctl(ap_net_dev, WLC_GET_CHANNEL, &ci, sizeof(ci), 0))) {
        printf("%s: get ConAP channel fail!\n", __FUNCTION__);
        error = -1;
        ptmp += snprintf(ptmp, 80, "%d|", error);
        ptmp += snprintf(ptmp, 80, " conap get channel fail");
        goto exit;        
    }else{
        printf("%s Got ConAP TargetChannel[%d]\n",__FUNCTION__,ci.target_channel);
        conap_channel = ci.target_channel;
        ptmp += snprintf(ptmp, 80, "%d|", conap_channel);
        ptmp += snprintf(ptmp, 80, " conap got targetchannel");
    }
exit:
	memset(buf, 0x0, len);
	memcpy(buf, tmp, len);
